...
# YOLO-World v2 RGB-IR Stable (With Text Update)
#
# 配置说明：启用 Text Update，禁用 Aggregator Fusion
#
# 数据流：
#   RGB → Image_Model ──┐
...
#   2. ❌ Aggregator 不与 fused_feats 融合 (fusion_type='none')
#
# 设计理念：
...
model = dict(
    ...
    # ⭐ Text Update V5 - 带 Class Mask
    text_update=dict(
        type='MultiScaleTextUpdateV5',
        in_channels=rgb_out_channels,   # [128, 256, 512]
        text_dim=text_channels,         # 512
        num_classes=num_classes,        # 4
        hidden_dim=text_hidden_dim,     # 256
        scale_init=text_scale_init,     # 0.0
        fusion_method=text_fusion_method,  # 'learned_weight'
        cross_batch=True,           # 跨 batch 聚合
        use_confidence_gate=False,  # 使用 Class Mask
        confidence_bias=0.0,        # 置信度偏置
    ),
