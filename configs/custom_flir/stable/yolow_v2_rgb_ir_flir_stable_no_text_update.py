...
# YOLO-World v2 RGB-IR Stable (No Text Update)
#
# 简化版配置 - 不使用 Text Update 模块
#
# 数据流：
#   RGB → Image_Model ──┐
...
#
# 设计理念：
#   文本语义作为"只读"先验知识指导视觉特征处理，但不被视觉信息反向更新
...
model = dict(
    ...
    backbone=dict(
        ...
        # ⭐ Text Update - 禁用（不设置或设为None）
        # text_update=None,  # 显式禁用
    ...
    aggregator=dict(
        ...
        fusion_type='none',  # ⭐ 关键修改：不与 fused_feats 融合
    ),
