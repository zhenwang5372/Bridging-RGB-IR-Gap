...
# YOLO-World v2 RGB-IR Stable (With Aggregator Fusion)
#
# 配置说明：禁用 Text Update，启用 Aggregator Fusion
#
# 数据流：
#   RGB → Image_Model ──┐
...
#
# 与基础版的区别：
#   1. ❌ 禁用 Text Update 模块 - 使用原始 text_feats
#   2. ✅ Aggregator 与 fused_feats 融合 (fusion_type 可选)
#
# 设计理念：
#   类别特定特征聚合后与原始融合特征结合，保留更完整的视觉信息
...
model = dict(
    ...
    aggregator=dict(
        ...
        # ├────────────────────────────────────────────────────────────────┤
        # │ 'none'   : 不融合，直接返回聚合后的特征                          │
        # │ 'add'    : 简单相加，aggregated + fused                         │
        # │ 'concat' : Concat后用1x1卷积降维 (推荐)                         │
        # │            Combined = Concat([aggregated, fused], dim=1)       │
        # │            Final = Conv1x1(Combined)                           │
        # └────────────────────────────────────────────────────────────────┘
    ),
