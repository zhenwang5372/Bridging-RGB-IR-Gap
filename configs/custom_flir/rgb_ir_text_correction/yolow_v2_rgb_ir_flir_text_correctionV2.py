...
# YOLO-World v2 RGB-IR with Text-Guided IR Correction V2 for FLIR Dataset
#
# 核心改进（基于 ir_correction.markdown）：
# 1. 空间门控机制 (Spatial Gating): M_err 作为乘法 mask
# 2. Min-Max 归一化: 替代 Softmax 加权
# 3. 更强的物理可解释性: M_err ⊙ X_ir 直接提取错误特征
#
# 架构：
# - Backbone: 双流RGB-IR + Text-guided IR Correction V2 (空间门控)
# - Neck: SimpleChannelAlign (只做通道对齐)
# - 特点: 在fusion前先用空间门控机制纠正IR特征
...
# IR Correction V2 参数
correction_alpha = 0.3  # 纠错强度初始值（每个尺度可学习）
...
model = dict(
    ...
    backbone=dict(
        _delete_=True,
        type='DualStreamMultiModalYOLOBackboneWithCorrectionV4',  # ⭐ 使用V2版本
        # RGB backbone
        image_model=dict(
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        ...
        custom_keys={
            ...
            'backbone.ir_correction': dict(lr_mult=1.0),     # IR纠错模块V2正常学习率
