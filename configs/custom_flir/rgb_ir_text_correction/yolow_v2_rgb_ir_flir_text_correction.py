...
# YOLO-World v2 RGB-IR with Text-Guided IR Correction for FLIR Dataset

# 核心设计：
# - Backbone: 双流RGB-IR + Text-guided IR Correction (新增)
# - Neck: SimpleChannelAlign (只做通道对齐)
# - 特点: 在fusion前先用Text指导纠正IR特征

# 基于: yolow_v2_rgb_ir_flir_no_update.py
...
text_channels = 512
base_lr = 1.5e-3  # 稍微降低学习率，因为增加了新模块
weight_decay = 0.05 / 2
train_batch_size_per_gpu = 16
img_scale = (640, 640)
# IR Correction 参数
correction_alpha = 0.3  # IR纠错强度初始值
temperature = 0.5       # Softmax温度初始值
# ======================== Model Definition ========================
...
backbone=dict(
    _delete_=True,
    type='DualStreamMultiModalYOLOBackboneWithCorrection',  # ⭐ 使用新的Backbone
    # RGB backbone
    image_model=dict(
    ...
    # ⭐ 新增：Text-guided IR纠错模块
    ir_correction=dict(
        type='TextGuidedIRCorrection',
        rgb_channels=rgb_out_channels,  # RGB通道数 (P3, P4, P5)
        ir_channels=ir_out_channels,    # IR通道数 (P3, P4, P5)
        text_dim=text_channels,         # 文本维度
        num_classes=num_classes,        # FLIR类别数
        correction_alpha=correction_alpha,  # 纠错强度（每个尺度可学习）
        temperature=temperature,        # Softmax温度（每个尺度可学习）
    ),
...
norm_decay_mult=0.0,
custom_keys={
'backbone.text_model': dict(lr_mult=0.01),       # CLIP低学习率
'backbone.ir_model': dict(lr_mult=1.0),          # IR Backbone正常学习率
'backbone.ir_correction': dict(lr_mult=1.0),     # ⭐ IR纠错模块正常学习率
