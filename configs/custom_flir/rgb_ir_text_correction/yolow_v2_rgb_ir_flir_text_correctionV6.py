...
# YOLO-World v2 RGB-IR with Text-Guided IR Correction V6 for FLIR Dataset
#
# ╔═══════════════════════════════════════════════════════════════════════════╗
# ║                           V6 核心改动                                      ║
# ╠═══════════════════════════════════════════════════════════════════════════╣
# ║ 1. IR Correction 输出 M_err（语义错误图）供融合模块使用                    ║
# ║ 2. 融合模块使用 M_err 替代从 IR 学习的 attention_map                       ║
# ║ 3. 仍然 concat IR 特征进行跨模态融合                                       ║
# ║ 4. M_err 能够准确定位目标区域（基于 RGB-IR-Text 语义对比）                  ║
# ╚═══════════════════════════════════════════════════════════════════════════╝
#
# 融合公式:
#   x_rgb_attended = x_rgb * M_err_resized
...
# IR Correction V6 参数（与 V5 相同，但输出 M_err）
...
model = dict(
    ...
    backbone=dict(
        _delete_=True,
        type='DualStreamMultiModalYOLOBackboneWithCorrectionV6',  # ⭐ 使用 V6
        # RGB backbone
        image_model=dict(
        ...
        # ⭐ M_err 引导的 RGB-IR 融合 V1（用 M_err 替代 attention_map）
        fusion_module=dict(
            type='MultiLevelMerrGuidedFusionV1',
            rgb_channels=rgb_out_channels,
            ir_channels=ir_out_channels,
        ),
