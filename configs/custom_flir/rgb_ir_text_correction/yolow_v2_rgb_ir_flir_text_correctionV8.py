...
# YOLO-World v2 RGB-IR with Text-Guided IR Correction V8 for FLIR Dataset
#
# ╔═══════════════════════════════════════════════════════════════════════════╗
# ║                           V8 核心改动                                      ║
# ╠═══════════════════════════════════════════════════════════════════════════╣
# ║ 1. 完全不使用 IR 特征进行融合！                                            ║
# ║ 2. 只用 M_err（语义错误图）来增强 RGB 特征                                 ║
# ║ 3. ⭐ M_err 应用双阈值硬注意力（Hard Attention with Dual Thresholds）      ║
# ║    - M_err < threshold_low (0.2)  → 0  (抑制噪声)                         ║
# ║    - M_err > threshold_high (0.7) → 1  (强化目标)                         ║
# ║    - 中间区域保持原值（平滑过渡）                                           ║
# ║ 4. 相比 V7 的软注意力，V8 更聚焦目标区域                                   ║
# ╚═══════════════════════════════════════════════════════════════════════════╝
#
# 设计理念:
#   - M_err 通过 RGB-IR-Text 对比准确定位目标区域
#   - V7 使用软注意力（M_err 原值），可能包含噪声
#   - V8 使用硬注意力，明确区分"目标 vs 非目标"
#   - 双阈值策略平衡"去噪"和"保留边界"
#
# 融合公式:
#   M_err_hard = hard_threshold(M_err, low=0.2, high=0.7)
#   x_rgb_attended = x_rgb * M_err_hard         (用硬注意力加权 RGB)
...
model = dict(
    ...
    backbone=dict(
        _delete_=True,
        type='DualStreamMultiModalYOLOBackboneWithCorrectionV6',  # ⭐ 使用 V6 Backbone
        # RGB backbone
        image_model=dict(
        ...
        # IR backbone (⭐ 注意：仍需要 IR 来生成 M_err，但不用于融合)
        ir_model=dict(
        ...
        # ⭐ Text-guided IR纠错模块 V6（输出 M_err）
        ir_correction=dict(
            type='TextGuidedIRCorrectionV6',
        ...
        # ⭐ M_err 引导的 RGB 增强 V3（硬注意力 + 双阈值）
        fusion_module=dict(
            type='MultiLevelMerrGuidedFusionV3',
            rgb_channels=rgb_out_channels,
            ir_channels=ir_out_channels,  # 保留兼容，但不使用
            threshold_low=threshold_low,   # ⭐ 低阈值
            threshold_high=threshold_high, # ⭐ 高阈值
        ),
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        ...
        custom_keys={
            ...
            'backbone.fusion_module': dict(lr_mult=1.0),
