...
# YOLO-World v2 RGB-IR with Text-Guided IR Correction V7 for FLIR Dataset
#
# ╔═══════════════════════════════════════════════════════════════════════════╗
# ║                           V7 核心改动                                      ║
# ╠═══════════════════════════════════════════════════════════════════════════╣
...
# ║ 3. M_err 明确指出目标区域，避免 IR 语义污染                                ║
# ║ 4. 适用于 IR 语义错误严重的场景（如 P3/P4 背景高亮问题）                    ║
# ╚═══════════════════════════════════════════════════════════════════════════╝
#
# 设计理念:
#   - M_err 通过 RGB-IR-Text 对比准确定位目标区域
#   - IR 特征可能存在语义错误（背景高亮等）
#   - 直接用 M_err 增强 RGB 目标区域，避免 IR 污染
#
# 融合公式:
...
model = dict(
    ...
    backbone=dict(
        ...
        # ⭐ M_err 引导的 RGB 增强 V2（完全不使用 IR 特征！）
        fusion_module=dict(
            type='MultiLevelMerrGuidedFusionV2',
            rgb_channels=rgb_out_channels,
