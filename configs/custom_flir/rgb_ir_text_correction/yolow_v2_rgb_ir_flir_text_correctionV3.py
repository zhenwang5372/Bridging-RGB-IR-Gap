...
# YOLO-World v2 RGB-IR with Text-Guided IR Correction V3 for FLIR Dataset
#
# V3 新增功能：
# - 在验证阶段自动打印 Alpha 参数到日志
# - 便于分析 Alpha 与 mAP 的关系
#
# 架构（与 V2 相同）：
# - Backbone: 双流RGB-IR + Text-guided IR Correction V3 (带 Alpha 日志)
...
log_alpha = True  # ⭐ V3 新增：是否打印 Alpha 到日志
log_interval = 50  # ⭐ V3 新增：训练时打印 Alpha 的间隔（iteration 数）
...
model = dict(
    ...
    backbone=dict(
        _delete_=True,
        type='DualStreamMultiModalYOLOBackboneWithCorrectionV3',  # ⭐ 使用V3版本
        # RGB backbone
        image_model=dict(
        ...
        # ⭐ Text-guided IR纠错模块 V3 (带 Alpha 日志输出)
        ir_correction=dict(
            type='TextGuidedIRCorrectionV3',
            rgb_channels=rgb_out_channels,
            ir_channels=ir_out_channels,
            text_dim=text_channels,
            num_classes=num_classes,
            correction_alpha=correction_alpha,
            log_alpha=log_alpha,  # ⭐ V3 新增：启用 Alpha 日志
            log_interval=log_interval,  # ⭐ V3 新增：每 50 iter 打印一次
        ),
