...
# YOLO-World v2 RGB-IR with Text-Guided IR Correction V5 Plus for FLIR Dataset
#
# V5 Plus 核心改动：
# - 多尺度动态投影维度 embed_channels（参考 YOLO-World 的设计）
#   - P3: 128, P4: 256, P5: 512
# - 使用 LiteFFTIRBackboneV3（深度可分离卷积 + base_channels=64）
#   - 输出通道 [128, 256, 512] 与 RGB 对齐
# - 保持 V5 的余弦相似度机制
# - Alpha 初始值保持 -0.3
# - 随机种子: 3407
...
# IR Correction V5 Plus 参数
correction_alpha = -0.3  # ⭐ V5Plus: 初始值 -0.3（负值）
cosine_scale = 10.0      # ⭐ V5Plus: 余弦相似度缩放因子
log_alpha = True
log_interval = 50

# ⭐ V5Plus 新增：多尺度投影维度（参考 YOLO-World）
# YOLO-World: neck_embed_channels = [128, 256, last_stage_out_channels // 2] = [128, 256, 512]
embed_channels = [128, 256, 512]  # P3, P4, P5

# ======================== Model Definition ========================
# RGB backbone output channels (YOLOv8s with widen_factor=0.5)
rgb_out_channels = [128, 256, 512]

# IR backbone output channels (LiteFFTIRBackboneV3 with base_channels=64)
# ⭐ V5Plus: 使用 base_channels=64，输出 [128, 256, 512] 与 RGB 对齐
...
model = dict(
    ...
    backbone=dict(
        _delete_=True,
        type='DualStreamMultiModalYOLOBackboneWithCorrectionV5Plus',  # ⭐ 使用V5Plus版本
        # RGB backbone
        image_model=dict(
        ...
        # ⭐ V5Plus: 使用 LiteFFTIRBackboneV3（深度可分离卷积 + base_channels=64）
        ir_model=dict(
            type='LiteFFTIRBackboneV3',
            in_channels=3,
            base_channels=64,  # ⭐ V5Plus: 64 → 输出 [128, 256, 512] 与 RGB 对齐
            out_indices=(0, 1, 2),
            frozen_stages=-1,
            freq_ratio=0.5,
        ),
        
        # ⭐ Text-guided IR纠错模块 V5 Plus (多尺度动态投影)
        ir_correction=dict(
            type='TextGuidedIRCorrectionV5Plus',
            rgb_channels=rgb_out_channels,     # [128, 256, 512]
            ir_channels=ir_out_channels,       # [128, 256, 512]
            text_dim=text_channels,            # 512
            embed_channels=embed_channels,     # ⭐ V5Plus: [128, 256, 512]
            num_classes=num_classes,
            correction_alpha=correction_alpha, # -0.3
            cosine_scale=cosine_scale,         # 10.0
            log_alpha=log_alpha,
            log_interval=log_interval,
        ),
        
        # RGB-IR fusion
        # ⭐ V5Plus: 因为 IR 和 RGB 通道数已对齐，fusion 可以更简单
        fusion_module=dict(
