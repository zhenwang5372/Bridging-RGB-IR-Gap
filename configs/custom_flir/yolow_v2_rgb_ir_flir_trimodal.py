...
# YOLO-World v2 RGB-IR Trimodal Configuration for FLIR Dataset
# 使用TriModalPhasedNeck进行三模态融合
...
model = dict(
    ...
    backbone=dict(
        _delete_=True,
        type='DualStreamMultiModalYOLOBackboneV2',
        image_model=dict(
    ...
    neck=dict(
        _delete_=True,
        type='TriModalPhasedNeck',
        in_channels=[128, 256, 512],
        text_dim=text_channels,
        hidden_dim=256,
        use_multiscale_text_update=True,  # 使用多尺度Text更新
        multiscale_fusion_first=True,
        ir_correction_cfg=dict(
            temperature=0.07,
        ),
        rgb_enhancement_cfg=dict(
            agreement_temperature=0.1,
        ),
#         # 第1层开关：是否使用多尺度
# use_multiscale_text_update = True/False

# # 第2层开关：多尺度的实现方式
# multiscale_fusion_first = True/False

# # 第3层开关：具体的融合/更新方法
# fusion_method = 'fpn'/'concat'/'attention'/'deformable'  # fusion_first=True时
# fusion_method = 'equal'/'learned_weight'/'attention'     # fusion_first=False时
        text_update_cfg=dict(
            gamma=0.1,
            cbam_reduction=16,
            fusion_method='fpn',  # 可选: 'equal', 'learned_weight', 'attention'
        ),
    ),
    bbox_head=dict(
        type='YOLOWorldHead',
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        ...
        custom_keys={
            ...
            'neck.ir_correct': dict(lr_mult=1.0),
            'neck.rgb_enhance': dict(lr_mult=1.0),
            'neck.text_update': dict(lr_mult=1.0),
