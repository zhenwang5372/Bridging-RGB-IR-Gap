...
# YOLO-World v2 RGB-IR with Class-Specific Features for FLIR Dataset

# 核心设计（基于text_correction的改进）：
# - Backbone: 双流RGB-IR + Text-guided IR Correction（保留）
# - Fusion: MultiLevelRGBIRFusion（保留，必须使用）
# - 阶段2: Text-guided RGB Enhancement（新增）
#   - 用Fused features计算attention
#   - 用RGB features生成类别特定特征
# - 阶段3: Multi-scale Text Update（新增）
#   - 跨尺度融合视觉证据
#   - YOLO-World风格的残差更新
# - Head: Class-Specific Detection（修改）
#   - Region-Text相似度分类
#   - 类别独立的边界框预测
...
model = dict(
    ...
    backbone=dict(
        _delete_=True,
        type='DualStreamMultiModalYOLOBackboneWithClassSpecific',  # ⭐ 新Backbone
        ...
        # ⭐ 新增：RGB Enhancement（阶段2）
        rgb_enhancement=dict(
            type='TextGuidedRGBEnhancement',
            rgb_channels=rgb_out_channels,
            text_dim=text_channels,
            num_classes=num_classes,
            d_k=d_k,
    ...
    bbox_head=dict(
        type='YOLOWorldHead',
        head_module=dict(
            type='ClassSpecificYOLOHeadModule',
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        ...
        custom_keys={
            ...
            'backbone.rgb_enhancement': dict(lr_mult=1.0),  # ⭐ 新模块
            'backbone.text_update': dict(lr_mult=1.0),      # ⭐ 新模块
