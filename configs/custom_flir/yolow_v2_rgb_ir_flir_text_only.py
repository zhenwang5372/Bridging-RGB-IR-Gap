...
# YOLO-World v2 RGB-IR Text-Only Configuration for FLIR Dataset
# 极简版：只更新Text，RGB和IR保持原样
...
custom_imports = dict(
    imports=['yolo_world', 'yolo_world.text_only_register'],  # 使用极简版注册
...
model = dict(
    ...
    neck=dict(
        _delete_=True,
        type='TextOnlyUpdateNeck',  # 极简版Neck（只更新Text）
        in_channels=[128, 256, 512],
        text_dim=text_channels,
        hidden_dim=256,
        use_multiscale_text_update=True,  # 使用多尺度Text更新
        multiscale_fusion_first=True,     # 先融合再更新（推荐）
        # 只需要 text_update_cfg，不需要 ir_correction_cfg 和 rgb_enhancement_cfg
        text_update_cfg=dict(
            gamma=0.05,           # 降低Text更新强度
            cbam_reduction=16,
            fusion_method='fpn',  # 'fpn', 'concat', 'attention', 'deformable'
            target_size=40,       # 融合后的特征图大小
        ),
    ),
    bbox_head=dict(
        type='YOLOWorldHead',
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        ...
        custom_keys={
            ...
            'backbone.ir_model': dict(lr_mult=1.0),  # 降低IR backbone学习率
            'neck.text_update': dict(lr_mult=1.0),   # Text更新模块降低10倍
            'gamma': dict(lr_mult=0.1, weight_decay=0.0),  # gamma特殊处理
