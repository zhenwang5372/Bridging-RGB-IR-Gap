# ============================================================================
# 消融实验 exp1_no_anchor3: Full Spatial Cross-Attention (No Text Anchor)
#
# 与 IR_RGB_Merr_Cons 的对照：
#   - 去掉 text 语义引导
#   - Q = proj(X_ir) 全空间, K = proj(X_rgb) 全空间 (不池化)
#   - O(H²W²) 全空间注意力矩阵
#   - 单一 gamma + 单一 estimator
#   - ir_corrected = x_ir * (1 + gamma * estimator(x_ir * response_map))
#
# 注意: P3 层 (160×160) 会产生 25600×25600 的注意力矩阵，显存消耗极大
# ============================================================================
...
model = dict(
    ...
    backbone=dict(
        ...
        # ★ 消融: Full Spatial Cross-Attention (No Text Anchor)
        ir_correction=dict(
            type='IR_RGB_CrossModal_FullCrossAttn',
