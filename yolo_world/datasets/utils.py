...
def yolow_collate(data_batch: Sequence,
    ...
    for i in range(len(data_batch)):
        ...
        # Handle IR inputs
        if 'inputs_ir' in data_batch[i]:
            batch_imgs_ir.append(data_batch[i]['inputs_ir'])
            has_ir = True
        elif hasattr(datasamples, 'img_ir') and datasamples.img_ir is not None:
            batch_imgs_ir.append(datasamples.img_ir)
            has_ir = True
        ...
        bboxes_labels = torch.cat((batch_idx, gt_labels[:, None].float(), gt_bboxes),
                                  dim=1)
        batch_bboxes_labels.append(bboxes_labels)

    collated_results = {
        'data_samples': {
            'bboxes_labels': torch.cat(batch_bboxes_labels, 0) if batch_bboxes_labels else torch.zeros(0, 6)
        }
    }
    ...
    # Stack IR inputs
    if has_ir and len(batch_imgs_ir) > 0:
        if use_ms_training:
            collated_results['inputs_ir'] = batch_imgs_ir
        else:
            collated_results['inputs_ir'] = torch.stack(batch_imgs_ir, 0)
