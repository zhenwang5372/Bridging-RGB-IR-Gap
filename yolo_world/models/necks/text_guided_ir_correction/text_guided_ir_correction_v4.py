...
# Text-guided IR Correction Module V4
# 
# 基于 V3，核心改动：
# - Alpha 初始值改为 -0.3（负值表示增强而非抑制）
...
class TextGuidedIRCorrectionV4(BaseModule):
    """
    Text-guided IR Correction Module V4
    
    基于 V3，核心改动：
    - Alpha 初始值改为 -0.3（负值：增强错误区域特征）
    - 公式变为：X_ir^corrected = X_ir - (-0.3) × Error_map = X_ir + 0.3 × Error_map
    ...
    Args:
        ...
        correction_alpha (float): 纠错强度初始值，默认 -0.3（⭐ V4 改动）
    ...
    def __init__(
        ...
        text_dim: int = 512,
        num_classes: int = 4,
        correction_alpha: float = -0.3,  # ⭐ V4 改动：默认值 -0.3
        log_alpha: bool = True,
        log_interval: int = 50,
        init_cfg=None
    ):
        ...
        for rgb_ch, ir_ch in zip(rgb_channels, ir_channels):
            self.correction_modules.append(
                SingleLevelTextGuidedCorrectionV4(
                    rgb_channels=rgb_ch,
                    ir_channels=ir_ch,
                    text_dim=text_dim,
...
class SingleLevelTextGuidedCorrectionV4(nn.Module):
    """
    单尺度的 Text-guided IR Correction V4
...
@MODELS.register_module()
class DualStreamMultiModalYOLOBackboneWithCorrectionV4(BaseModule):
    """
    带IR纠错的双流多模态YOLO Backbone V4
    
    基于 V3，核心改动：Alpha 初始值 -0.3
    """
