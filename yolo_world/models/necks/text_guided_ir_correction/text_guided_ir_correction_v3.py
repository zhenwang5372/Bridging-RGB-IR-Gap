...
# Text-guided IR Correction Module V3
# 
# 基于 V2，增加 Alpha 参数的日志输出功能
# 在验证阶段自动打印 Alpha 值到日志，便于分析 Alpha 与 mAP 的关系
...
class TextGuidedIRCorrectionV3(BaseModule):
    """
    Text-guided IR Correction Module V3
    
    基于 V2，增加以下功能：
    - 训练时每隔 N 个 iteration 打印 Alpha 参数
    - 验证时打印 Alpha 参数
    - 便于观察 Alpha 的变化趋势
    ...
    def __init__(
        ...
        for rgb_ch, ir_ch in zip(rgb_channels, ir_channels):
            self.correction_modules.append(
                SingleLevelTextGuidedCorrectionV3(
                    rgb_channels=rgb_ch,
                    ir_channels=ir_ch,
                    text_dim=text_dim,
    ...
    def reset_alpha_log_flag(self):
        ...
        self._alpha_logged_this_epoch = False
        self._train_iter_count = 0  # 也重置训练计数器
    ...
    def forward(
        ...
        # ⭐ V3 新增：打印 Alpha 值
        if self.training:
            # 训练时：每隔 log_interval 个 iteration 打印一次
...
class SingleLevelTextGuidedCorrectionV3(nn.Module):
    """
    单尺度的 Text-guided IR Correction V3
    
    与 V2 逻辑完全相同，仅类名不同以便区分
    """
...
@MODELS.register_module()
class DualStreamMultiModalYOLOBackboneWithCorrectionV3(BaseModule):
    """
    带IR纠错的双流多模态YOLO Backbone V3
    
    基于 V2，增加以下功能：
    - 在验证阶段开始时重置 Alpha 打印标志
    - 确保每个 epoch 只打印一次 Alpha 值
    """
    ...
    def train(self, mode: bool = True):
        ...
        ⭐ V3 新增：切换到训练模式时重置 Alpha 打印标志
        """
        super().train(mode)
        self._freeze_stages()
        
        # 切换到训练模式时，重置 Alpha 打印标志
        # 这样下次验证时会重新打印
    ...
    def forward(
        ...
        # IR Correction V3 (会在验证时自动打印 Alpha)
