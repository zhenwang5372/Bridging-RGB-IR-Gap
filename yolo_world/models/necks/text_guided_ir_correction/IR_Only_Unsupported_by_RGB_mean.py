...
# IR-Only Unsupported by RGB Module (Mean version)
# 
...
class IR_Only_Unsupported_by_RGB_mean(BaseModule):
    """
    IR-Only Unsupported by RGB Module (Mean version)
    ...
    def __init__(
        ...
        for rgb_ch, ir_ch in zip(rgb_channels, ir_channels):
            self.correction_modules.append(
                SingleLevelIROnlyUnsupportedByRGB_mean(
                    rgb_channels=rgb_ch,
                    ir_channels=ir_ch,
                    text_dim=text_dim,
    ...
    def log_alpha_values(self, stage: str = 'val', iter_num: int = None):
        ...
        if stage == 'train' and iter_num is not None:
            logger.info(
                f"[IR-Only Unsupported by RGB (Mean)] Iter [{iter_num}] Alpha: "
                f"P3={alphas['P3']:.6f}, P4={alphas['P4']:.6f}, "
                f"P5={alphas['P5']:.6f}, Mean={alphas['mean']:.6f}"
            )
        else:
            logger.info(
                f"[IR-Only Unsupported by RGB (Mean)] Val Alpha: "
                f"P3={alphas['P3']:.6f}, P4={alphas['P4']:.6f}, "
                f"P5={alphas['P5']:.6f}, Mean={alphas['mean']:.6f}"
            )
...
class SingleLevelIROnlyUnsupportedByRGB_mean(nn.Module):
    """
    单尺度的 IR-Only Unsupported by RGB (Mean version)
    ...
    def forward(
        ...
        # Step 2: IR-Only Unsupported by RGB（不使用 G，直接计算）
        # M_err = A_ir * (1 - A_rgb)
        ...
        # Step 3: 空间门控
        F_extracted = x_ir * M_err_spatial            #IR channels 
        Error_map = self.error_estimator(F_extracted) #IR channels  这两个地方可视化应该看不出效果的
...
@MODELS.register_module()
class DualStreamMultiModalYOLOBackboneWithIROnlyUnsupportedByRGB_mean(BaseModule):
    """
    带 IR-Only Unsupported by RGB (Mean) 纠错的双流多模态 YOLO Backbone
    """
